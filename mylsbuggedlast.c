...
int main(int argc, char * const argv[], const char *optstring){
	...
	//Les paramètres necessaire pour la boucle
	int fd, nread;
	char buf[BUF_SIZE];
	...
	while(1){
		...
		while (bpos<nread){
			...
			//Ajout à la liste des fichiers recursives
			...
			//On verifie qu'on a assez de memoire alouée
			if (arpos == arlen) {
				arlen *= 2;
	...
	//On délimite correctement la mémoire
	/*arlen = arpos;
	array = realloc(array, arlen * sizeof(char*));
*/
	//ERREUR traitée
	...
	//On commence le traitement en fonction des paramètres

	if(rflag == 1)
	{
		//Recursive
		//On parcourt l'ensemble de mes elements dans recArray, et j'arriche chaque element
